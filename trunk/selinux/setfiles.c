...
/* _FILE_OFFSET_BITS=64 is assumed in matchpathcon_filespec_add*/
...
#define OPTS_s    (1<<8)	/* s */
#define OPTS_v    (1<<9)	/* v */
#define OPTS_W    (1<<10)	/* W */
#define OPTS_e    (1<<11)	/* e */
#define OPTS_o    (1<<12)	/* o */
...
int setfiles_main(int argc, char *argv[])
{
	...
	opts = bb_getopt_ulflags(argc, argv, "Fc:dlnpqr:svWe:o:", &policyfile, &rootpathname, &exclude_dir, &outfilename, &verbose);
